...
def connect_ssh():
    ...
    try:
        ...
        session_id = request.sid  # Retrieve session ID from socketio connection
        terminal_namespace = TerminalNamespace('/terminal')
        terminal_namespace.ssh_connections[session_id] = ssh

        emit('data', '\r\n*** SSH CONNECTION ESTABLISHED ***\r\n', room=session_id)
        shell = ssh.invoke_shell()
        shell.setblocking(False)
        terminal_namespace.shell_connections[session_id] = shell

        socketio.start_background_task(terminal_namespace.listen_shell, session_id)
        return jsonify({'success': True})

    except paramiko.AuthenticationException as e:
        return jsonify({'error': f'SSH CONNECTION ERROR: {str(e)}'})
    except paramiko.SSHException as e:
        return jsonify({'error': f'SSH CONNECTION ERROR: {str(e)}'})
    except Exception as e:
        return jsonify({'error': f'SSH CONNECTION ERROR: {str(e)}'})
